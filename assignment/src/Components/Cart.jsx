...
const Cart = () => { 
    ...
    const calculateTotal = (cart) => {
        return cart.reduce((total, item) => total + item.price * (quantity[item.id] || 1), 0).toFixed(2);
      };  
    ...
    { 
      ...
      {cart.map((item) => (
        <div key={item.id} className="mb-6 bg-white p-4 md:p-6 shadow-md rounded-lg">
          ...
          <MyImage image={item.thumbnail} alt={item.title} width={"full"} classname={"rounded-lg mb-4 md:w-40 md:mb-0"}/>
