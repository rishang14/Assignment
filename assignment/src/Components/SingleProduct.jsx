...
const SingleProduct = () => { 
    ...
    useEffect(()=>{
        ...
        const SingleProduct= async ()=>{
            ...
            const {data: data}= await axios.get(`https://dummyjson.com/products/${id}`)  
            console.log(data)
             setproduct(data) 
             setloading(false) 
             setimg(data.images) 
     ...
     {loading ? (
        ...
        <div className="mt-[60px] mx-auto max-w-[75rem] px-0 py-[4rem]">  
            ...
            <div className='flex flex-col gap-6 lg:w-2/4'>
                <img src={activeImg} alt="" className='w-full h-[400px] aspect-square object-cover rounded-xl'/>
                 <div className='flex flex-row justify-between h-24'>
                    {/* <img src={img[0]} alt="" className='w-24 h-24 rounded-md cursor-pointer' onClick={() => setActiveImage(img[0])}/>
                    <img src={img[1]} alt="" className='w-24 h-24 rounded-md cursor-pointer' onClick={() => setActiveImage(img[1])}/>
            ...
            <div className='flex flex-col gap-4 lg:w-2/4'>
                ...
                <div className='flex flex-row items-center gap-12'>
                 ...
                 <button className='bg-violet-800 text-white font-semibold py-3 px-16 rounded-xl h-full'onClick={()=>handleAddToCartButton(product)}>Add to Cart</button>
                </div>
            </div>
        </div>
